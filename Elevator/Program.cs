...
List<ElevatorCar> elevatorBank = new();
for (int i = 0; i < elevators; i++)
{
    elevatorBank.Add(new ElevatorCar(i));
}

int t = 0;
List<Thread> elevatorThreads = new();
foreach (ElevatorCar elevator in elevatorBank) 
{
...
while (callQueue.Count > 0)
{
    ...
    while (elevatorBank[elevatorId++].elevatorState != ElevatorCar.ElevatorState.Ready) 
    { 
